...
train_vit_v5.py
----------------
Quinta iteración. Objetivo: superar el 95 % de aciertos sobre el conjunto
difícil (rostros sintéticos con psi = 1.0 y semillas nunca vistas).

PUNTO DE PARTIDA (v4)
---------------------
    aciertos 92.0 %   ·   AUC 0.9679   ·   detecta sintéticas 92.8 %

Con un AUC de 0.9679 ningún umbral alcanza el 95 %: el margen que quedaba
en la decisión ya se agotó. Lo que falta es discriminación.

CAMBIO PRINCIPAL — 256 píxeles
------------------------------
Hasta ahora se reducía a 224, descartando el 12 % de la información que
traía el conjunto público (publicado a 256x256). Entrenar directamente a
256 conserva ese detalle. Los rostros generados siguen el mismo camino
1024 -> 256 bicúbico que usó quien publicó el conjunto, de modo que
ninguna clase queda distinguible por su procesamiento.

Se descartó 384 px: ampliar imágenes ya reducidas no recupera detalle, lo
inventa, y habría dado a los rostros generados —que sí parten de 1024— una
nitidez que las auténticas no pueden tener. Ese desajuste habría creado un
atajo nuevo.

ViT-B/16 fue preentrenado a 224, así que sus codificaciones posicionales
cubren una rejilla de 14x14 parches. A 256 la rejilla es de 16x16, por lo
que hay que interpolarlas. Es el procedimiento estándar para cambiar la
resolución de un ViT y conserva el conocimiento preentrenado.

SE MANTIENE DE v4
-----------------
· Datos con psi estratificado (mitad en [0.85, 1.0])
· Augmentación idéntica para ambas clases (JPEG, brillo, desenfoque)
· Umbral ajustado sobre val_hard, aplicado sin cambios a test_hard
· Predicción promediada sobre la imagen y su espejo

Uso:
    python train_vit_v5.py [--seed N] [--epochs N] [--tag NOMBRE]
"""
...
# Debe fijarse ANTES de importar el worker, que lee la variable al cargarse.
import os
...
def parse_args():
    ...
    p.add_argument("--seed", type=int, default=42,
                    help="Semilla. Cambiarla produce un modelo distinto para el ensamble")
    p.add_argument("--tag", type=str, default="a",
                    help="Etiqueta del modelo: guarda en vit_v5_<tag>.pt")
    p.add_argument("--max-minutes", type=float, default=210)
    return p.parse_args()


def build_model(device, image_size=IMG_SIZE):
    """
    ViT-B/16 preentrenado, adaptado a una resolución distinta de 224.

    torchvision expone interpolate_embeddings() justamente para esto:
    reescala la rejilla de codificaciones posicionales de 14x14 a 16x16
    mediante interpolación bicúbica, conservando el token de clasificación.
    Sin este paso el modelo rechazaría entradas de 256 px.
    """
    ...
    if image_size != 224:
        ...
        state = interpolate_embeddings(image_size=image_size, patch_size=16,
                                        model_state=state)

    m = vit_b_16(weights=None, image_size=image_size)
    m.load_state_dict(state)
    m.heads.head = nn.Linear(m.heads.head.in_features, 2)

    n_patches = (image_size // 16) ** 2
    print(f"  ViT-B/16 a {image_size}px · {n_patches} parches · "
          f"{sum(p.numel() for p in m.parameters()):,} parámetros")
    return m.to(device)
...
def main():
    ...
    print("=" * 74)
    print("ENTRENAMIENTO v5 — 256 PX + AUGMENTACIÓN + TTA + UMBRAL")
    print("=" * 74)
    ...
    print(f"  Resolución: {IMG_SIZE}px · semilla {args.seed} · etiqueta '{args.tag}'")

    if not (PROCESSED / "val_hard").exists():
        raise FileNotFoundError(
            f"Falta {PROCESSED / 'val_hard'}. Ejecuta antes build_dataset_v4.py")
    ...
    best_ckpt = CKPT_DIR / f"vit_v5_{args.tag}.pt"
    log_path = RESULTS / f"training_log_v5_{args.tag}.csv"
    rows = []
    t_start = time.time()
    ...
    print(f"{'v4':<8} {'92.0 %':>11} {'0.9679':>9} {'92.8 %':>21}")
    if "test_hard" in evals:
        t = evals["test_hard"]["tuned"]
        print(f"{'v5-' + args.tag:<8} {t['accuracy'] * 100:>10.1f} % {t['auc_roc']:>9.4f} "
    ...
    summary = {
        ...
        "hyperparams": {
            ...
            "seed": args.seed, "image_size": IMG_SIZE,
            "augmentation": "flip, JPEG 70-100, brillo, contraste, saturación, desenfoque/realce",
            "dataset": "processed_v4 (256 px nativos, psi estratificado)",
        },
        "evaluations": evals,
    }
    with open(RESULTS / f"evaluation_summary_v5_{args.tag}.json", "w") as f:

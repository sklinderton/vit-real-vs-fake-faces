...
build_dataset_v4.py
--------------------
Cuarta versión del conjunto de entrenamiento. Corrige el desajuste que
todavía quedaba entre la distribución de entrenamiento y la de evaluación.

DIAGNÓSTICO
-----------
El entrenamiento v4 alcanzó 92.0 % de aciertos con AUC 0.9679 sobre el
conjunto difícil. El ajuste de umbral ya está exprimido: con ese AUC,
ningún punto de corte llega al 95 %. Lo que falta es capacidad de
discriminación, no una decisión mejor.

Se descartó entrenar a 384 píxeles. Las imágenes del conjunto público
fueron publicadas ya reducidas a 256x256, de modo que ampliarlas a 384
no recupera detalle: lo interpola. Peor aún, los rostros que generamos
sí parten de 1024x1024 reales, así que a 384 tendrían detalle genuino
frente al detalle inventado de las de Kaggle. Como todas las auténticas
provienen de Kaggle, el modelo aprendería "nitidez a 384 = generada",
que es precisamente el tipo de atajo que costó tres versiones eliminar.

CORRECCIÓN
----------
Se procesa a 256 píxeles en lugar de 224. Es la resolución nativa del
conjunto público, así que no se descarta el 12 % de información que se
venía perdiendo, y los rostros generados siguen el mismo camino
1024 -> 256 que usó quien publicó el conjunto. Ninguna clase queda con
una procedencia distinguible.

Se mantiene la estratificación de psi introducida en la versión anterior:
...
OUT_DIR = PROJECT_ROOT / "data" / "processed_v4"
...
SEED_GEN_START = 2_000_000       # pool de entrenamiento
SEED_VALHARD_START = 2_500_000   # validación difícil
SEED_TESTHARD_START = 2_600_000  # prueba difícil
...
def generate(G, seeds, psis, device, desc):
    ...
    for seed, psi in tqdm(list(zip(seeds, psis)), desc=desc, unit="img"):
        ...
        # Mismo camino que siguió el conjunto público: 1024 -> 256 bicúbico.
        # No hay reducción adicional, así que ninguna clase queda más suave
        # que la otra por el procesamiento.
...
def main():
    ...
    print("=" * 72)
    print("DATASET v4 — 256 PX NATIVOS + PSI ESTRATIFICADO")
    print("=" * 72)
    print(f"  Device: {device}")
    print(f"  Resolución: {IMG_SIZE}x{IMG_SIZE} px")
    ...
    manifest = {
        "description": "Dataset v4 — 256 px nativos, psi estratificado",
        "random_seed": SEED,
    ...
    with open(META_DIR / "seeds_manifest_v4.json", "w") as f:
        json.dump(manifest, f, indent=2)

    print("\n" + "=" * 72)
    print("DATASET v4 LISTO")
    print("=" * 72)
